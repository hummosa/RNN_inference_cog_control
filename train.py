    ...
    while not done:
        ...
        if _use_oracle: 
            ...
            if thalamus_level_II: # use the same value for all timesteps, higher level thalamus, slower timescale.
                ...
                thalamic_inputs = torch.cat((thalamic_inputs_buffer, thalamic_inputs_current), axis=0)
            if config.optimize_only_lower_level_latent: # a different experiment. override the above and use the optimized values for the :2 in the last dimm, and uniform for the rest
                # use the optimized values for the :2 in the last dimm, and uniform for the rest
                thalamic_inputs_current = (model.thalamus.detach()[-1:]).float().to(model.device)
                thalamic_inputs_buffer = torch.from_numpy(np.stack(logger.timestep_data['thalamus'][-logger.horizon+1:]).squeeze(1)).float().to(model.device)
                thalamic_inputs = torch.cat((thalamic_inputs_buffer, thalamic_inputs_current), axis=0)
                # thalamic_inputs = (thalamic_inputs[:,:,:2], torch.ones(thalamic_inputs.shape[0], thalamic_inputs.shape[1], config.thalamus_size-2).float().to(model.device)/config.thalamus_size), axis=-1)
                # just replace the -1 timestep last dim 2: with uniform
                thalamic_inputs[-1:,:,2:] = torch.ones(thalamic_inputs.shape[1], config.thalamus_size-2).float().to(model.device)/config.thalamus_size
